...
async function fetchNews(category = '') {
    ...
    try {
        ...
        allArticles = newsData.results.map(article => ({
...
function renderNewsGrid() {
    ...
    let articlesToShow = isBookmarkView ? bookmarkedArticles : allArticles;

    if (!isBookmarkView && currentCategory !== 'all') {
        articlesToShow = articlesToShow.filter(article =>
            article.category.toLowerCase() === currentCategory.toLowerCase()
        );
    }

    if (!isBookmarkView && searchQuery) {
        articlesToShow = articlesToShow.filter(article =>
